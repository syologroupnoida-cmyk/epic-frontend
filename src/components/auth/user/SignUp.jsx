...
export default function UserSignUp() {
  ...
  async function dummyUserSignUpApi(payload) {
  ...
  return (
    <AuthShell
      mode="signup"
      eyebrow="New Account"
      title="Start planning with clearer choices"
      text="Create a profile for vendor shortlists, quote requests, and wedding ideas."
    >
        <form onSubmit={handleSubmit} className="auth-panel w-full rounded border border-white/40 p-6 shadow-2xl sm:p-8">
          <h2 className="font-serif text-3xl font-bold">User Sign up</h2>
          <div className="mt-5 grid gap-2 sm:grid-cols-2">
            ...
            <button className="h-12 rounded bg-[#b61f4b] text-sm font-bold text-white hover:bg-[#981a3f] sm:col-span-2">Create Account</button>
            {status ? <p className="text-sm font-bold text-[#24784f] sm:col-span-2">{status}</p> : null}
            <p className="text-center text-sm text-stone-600 sm:col-span-2">Already registered? <Link href="/user/login" className="font-bold text-[#b61f4b]">Login</Link></p>
          </div>
        </form>
    </AuthShell>
  );
