...
export default function UserLogin() {
  ...
  async function dummyUserLoginApi(payload) {
  ...
  return (
    <AuthShell
      ...
      title="Welcome back to your celebration"
      text="Manage shortlists, inquiries, and planner conversations from one elegant account."
    >
        <form onSubmit={handleSubmit} className="auth-panel w-full rounded border border-white/40 p-6 shadow-2xl sm:p-8">
          <h2 className="font-serif text-3xl font-bold">User Login</h2>
          <div className="mt-6 grid gap-4">
            ...
            <p className="text-center text-sm text-stone-600">New here? <Link href="/user/sign-up" className="font-bold text-[#b61f4b]">Create user account</Link></p>
          </div>
        </form>
    </AuthShell>
  );
